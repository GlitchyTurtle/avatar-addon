...
export function hitEvent(eventData) {
			...
			for (let enchants in MinecraftEnchantmentTypes) {
				...
				if (enchant_data.type.id === "baneOfArthropods" && item.id != "minecraft:enchanted_book") {				
					if (entity.hasTag('air')) {
						try { hitEntity.runCommand(`effect @s levitation 1 1 true`); } catch (error) {}
						...
						try { hitEntity.runCommand(`summon evocation_fang ~~~`); } catch (error) {}
						try { hitEntity.runCommand(`particle a:earth_shockwave ~~~`); } catch (error) {}
						...
						try { hitEntity.runCommand(`effect @s slowness 1 ${enchant_data.level} true`); } catch (error) {}
						try { hitEntity.runCommand(`particle a:water_wave ~~~`); } catch (error) {}
	...
	if (getScore("combat", entity) === 0) {
		entity.runCommand(`tellraw @s {"rawtext":[{"text":"§cYou are in combat! Do not leave the game!"}]}`);
		hitEntity.runCommand(`tellraw @s {"rawtext":[{"text":"§cYou are in combat! Do not leave the game!"}]}`);
	}
	entity.runCommand(`scoreboard players add @s combo 1`);
	hitEntity.runCommand(`scoreboard players set @s combo 0`);
	entity.runCommand(`scoreboard players set @s combat 220`);
	hitEntity.runCommand(`scoreboard players set @s combat 220`);
        ...
        if (getScore("combo", entity) > 5) {
            try {
				if (!hitEntity.hasTag("avatar") && !hitEntity.hasTag("earth") && !hitEntity.hasTag("air") && !hitEntity.hasTag("fire") && !hitEntity.hasTag("water")) {
					...
					entity.runCommand(`scoreboard players add @s sub_level 1`);
					entity.runCommand(`tellraw @s {"rawtext":[{"text":"§3Blocked ${hitEntity.name}'s Chi for ${getScore("level", entity)} seconds. They aren't a bender, so it just slows them down."}]}`);
