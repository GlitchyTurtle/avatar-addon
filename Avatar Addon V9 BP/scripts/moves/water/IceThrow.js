...
const command = {
    ...
    execute(player) {
		...
		player.runCommand("summon a:ice_block ^^0.3^1");
        ...
        let iceThrowTick = world.events.tick.subscribe(event => {
			...
			if (event.currentTick - startTick > 100) {
				try { player.runCommand(`tag @e[r=10,c=1,type=a:ice_block] add launch`); } catch (error) {}
			}
			try { player.runCommand(`tp @e[c=1,type=a:ice_block,tag=!launch] ^^0.3^1.5 facing @p`); } catch (error) {}
			if (getScore("detect_left", player) === 1) { 
				try { player.runCommand(`tag @e[r=10,c=1,type=a:ice_block] add launch`); } catch (error) {}
			}
			try {
				player.runCommand(`execute @e[c=1,type=a:ice_block,tag=launch] ~~~ tp @s ^^^-1`);
				player.runCommand(`execute @e[c=1,type=a:ice_block,tag=launch] ~~~ execute @e[r=2,name=!${player.nameTag},type=!a:ice_block] ~~~ event entity @e[r=5,type=a:ice_block] minecraft:explode`);
			} catch (error) {}
			try { player.runCommand(`testfor @e[type=a:ice_block,r=50,c=1]`); } catch (error) {
