export function help(message) {
	...
	player.runCommand(`tellraw @s {"rawtext":[{"text":"§b!shop <buy, sell, help>§r - Use the shop to sell and buy items, use <!shop help> to find out more"}]}`);
	player.runCommand(`tellraw @s {"rawtext":[{"text":"§b!info <moves, slots, passives>§r - Get your current moves or slots to choose from!"}]}`);
	player.runCommand(`tellraw @s {"rawtext":[{"text":"§b!home <set, delete, teleport, help>§r - Creates or edits a home, which you can tp back too."}]}`);
	player.runCommand(`tellraw @s {"rawtext":[{"text":"-----------------------------------------§r"}]}`);
}