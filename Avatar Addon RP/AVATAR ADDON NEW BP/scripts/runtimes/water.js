...
export function waterRuntime(player) {
	...
	if (getScore("detect_water", player) === 1) {
		...
		if (playerHasSkill(player, "Water Conduit")) player.addEffect("conduit_power", 5, { amplifier: 0, showParticles: false });
	}
	...
	if (getScore("detect_sneak", player) == 1 || player.hasTag("hiddenWater") || (player.hasTag("permHiddenWater"))) {
		...
		if (getScore("detect_sprint", player) == 1 && getScore("earth_sprint", player) < 100 && WATER_LOADED > 0 && playerHasSkill(player, "Water Jet Rush")) {
