...
function nonScrollOperations(source, itemName, itemAmount) {
	...
	if (itemName == "a:empty_cup") {
        const blockInFront = source.getBlockFromViewDirection({ includeLiquidBlocks: true, includePassableBlocks: true, maxDistance: 6 });
        const isLookingAtWater = (blockInFront && blockInFront.block.typeId == "minecraft:water");
...
export function beforeItemUse(eventData) {
    ...
    if (!(source instanceof Player) || source.hasTag("in_paisho_game")) return;
