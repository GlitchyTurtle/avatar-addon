...
const command = {
    name: 'Air Plume',
    description: 'Get the high ground on a pillar of air!',
    style: 'air',
    unlockable: 0,
    unlockable_for_avatar: 0,
    cooldown: 'fast',
    skill_required: "Air Plume",
    execute(player) {
        // Setup
        ...
        delayedFunc(player, (airArtillery) => {
            ...
            const sched_ID = system.runInterval(function tick() {
                ...
                // Spawn the particle
                const { x, y, z } = player.location;
                for (let i = 1; i < Math.min(distance * 3, 40); i++) {
                    map.setVector3("variable.plane", { x: 0.3, y: 15, z: 2 * i/3 });
                    player.dimension.spawnParticle("a:air_shockwave_dynamic", {x: x, y: y + i/3 - distance, z: z}, map);
                }
