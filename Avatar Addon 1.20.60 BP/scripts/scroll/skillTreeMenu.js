...
import { validateMoves } from './chooseSlotsMenu.js';
import { SkillTreeData } from "./../extensions/forms";
import { PLAYER_DATA_MAP } from "./../runtimes/main.js";
    ...
    "Non-bender": {
        ...
        children: {
            ...
            "Fleet Feet": {
                title: "Fleet Feet",
                description: [
                    'You have studied the way \nairbenders move, and somewhere \nalong the way perfected the \nart of silence.',
                    '§r§8§oSkulk Sensors won\'t notice you, \nand you can move through berry \nbushes and magma blocks.'
                ],
                icon: "textures/ui/avatar/skill_tree/light_footed",
                index: 12,
                ...
                children: {
                    "Acrobatics": {
                        title: "Acrobatics",
                        description: [
                            'Growing up in a circus has it\s perks!',
                            '§r§8§oPassive Speed I and Jump \nBoost I'
                        ],
                        icon: "textures/ui/avatar/skill_tree/skys_grace",
                        index: 20,
...
export function refreshSkillTree(source) {
    const playerSkills = [];
    ...
    for (let i = 0; i < buttonArray.length; i++) {
        ...
        if (isLockedOrder[i]) {
            playerSkills.push(btn[1]);
        }
    }

    PLAYER_DATA_MAP[source.id].skills = playerSkills;
}
...
export function skillTreeMenu(source, errorMsg = undefined) {
    ...
    form.button(61, "§6Reforge Tree", [`§r§7If you want to switch \nbuilds, you can just\nclick this button! It \ncosts 20 EXP Levels!`], "textures/ui/avatar/skill_tree/respec");
    ...
    form.show(source).then(async response => {
        ...
        warningMenu.show(source).then((ActionFormResponse) => {
            ...
            if (selection === 1) {
                ...
                PLAYER_DATA_MAP[source.id].skills.push(node.title);

                addNeededTags(source, node.title)
                
                // This needs to be done for Toph Armor, since order matters here
                if (!(source.hasTag("earth") && playerHasSkill(source, "Toph Armor") && node.title == "Chi Infusion+")) {
                    addNeededEvents(source, node.title);
                }

                source.sendMessage(`§7You chose the ${node.title} skill!`);
...
function resetSkillTree(player) {
    ...
    const warningMenu = new ActionFormData()
    .title("Warning!")
    .body(`§6Are you sure you want to do this?\n\n§7This will reset your skill tree and grant you back the essence you need, but costs 20 EXP Levels!\n `)
    .button("Confirm")
    .button("Cancel");
    ...
    warningMenu.show(player).then((ActionFormResponse) => {
        ...
        if (selection === 1) {
            ...
            // Level check
            if (player.level < 20) return player.sendMessage("§cYou need at least 20 EXP Levels for this!");
            player.addLevels(-20)

            playSound(player, 'random.levelup', 1, player.location, 1);
            player.sendMessage("§7You reset your skill tree!");

            // Make sure their score is valid before we tally up points to return
            let skillTreeEv = getScore("skill_tree", player);
            if (!skillTreeEv ||  !parseInt(skillTreeEv) || skillTreeEv <= 0) {
                console.warn(`Encountered a minor error when parsing ${source.name}'s skill tree score!`);
                setScore(player, "skill_tree", 1, false);
            ...
            // Bug Fix (notdiverseplatinum) -> Reseting your skill tree doesn't remove already equipped skill tree moves. 
            // DO NOT REMOVE THIS LINE
            validateMoves(player);

            // Countup and return skill points (or essences, as they are called ingame)
            const isLockedOrder = loadSkillTree(skillTreeEv);
            const trueCount = isLockedOrder.filter(value => value === true).length - 1;
            setScore(player, "skill_points", trueCount, true);

            // Reset the skill tree to the binary value of ...001, thus just the first node unlocked
            setScore(player, "skill_tree", 1, false);
