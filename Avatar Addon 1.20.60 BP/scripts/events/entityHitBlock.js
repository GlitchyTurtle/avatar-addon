...
import { setScore, getScore, playSound, calculateDistance, setCooldown, getCooldown } from "../util.js";
...
export function entityHitBlockEvent(eventData) {
    ...
    if (playerHasSkill(damagingEntity, "Pillar Pound") && (hitBlock.typeId === "minecraft:grass") && getScore("detect_sneak", damagingEntity) == 0 && getCooldown(damagingEntity) == 100) {
        setCooldown(damagingEntity, 95);
    ...
    } else if (playerHasSkill(damagingEntity, "Pillar Pound") && (hitBlock.typeId === "minecraft:grass") && getScore("detect_sneak", damagingEntity) == 1 && getCooldown(damagingEntity) == 100) {
        setCooldown(damagingEntity, 95);
        damagingEntity.onScreenDisplay.setTitle(`a:earth_super_fast`);
        damagingEntity.addTag("super_fast_cooldown");
