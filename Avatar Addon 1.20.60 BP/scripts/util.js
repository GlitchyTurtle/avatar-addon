...
import { mainSetCooldown, mainGetCooldown } from "./runtimes/main.js";
...
export function setCooldown(player, score = 0) {
    mainSetCooldown(player, score);
}

export function getCooldown(player) {
    return mainGetCooldown(player);
}
 ...
 */
export function setScore(entity, objectiveId, score, stack) {
...
export function applyBasicDamage(player, target, damageType, knockbackForce = 0, bypassArmor = false) {
    ...
    if (target instanceof Player) {
        // Line of sight check
        if (!isLineClear(player, player.getHeadLocation(), target.getHeadLocation()) && !isLineClear(player, player.location, target.location) && calculateDistance(player.location, target.location) > 5) return;
        ...
        if ((blockWindow && blockWindow > 2 && blockWindow < 27)) {
            ...
            target.addEffect("strength", 10, { amplifier: 2, showParticles: true });

            if ((target.hasTag("earth") || target.hasTag("avatar")) && playerHasSkill(target, "Neutral Jing")) {
        ...
        if (playerHasSkill(target, "Bending Resistance")) amountOfDamage *= 0.9
        if (playerHasSkill(target, "Bending Resistance+")) amountOfDamage *= 0.9

        if (playerHasSkill(target, "Bending Resistance++") && amountOfDamage < 2.1) {
            if (!target.hasTag("diaOff")) target.sendMessage("§7Bending Resistance++ skill has protected you!")
        } else {
        ...
        try {
            target.applyDamage(amountOfDamage * 2.7, { cause: damageCause, damagingEntity: player});
            if (knockbackForce <= 0) return;
            target.applyKnockback(playerViewDir.x, playerViewDir.z, knockbackForce, knockbackForce/4);
        } catch (error) {}
    }
...
export function enterCombatMode(player, target) {
	...
	setScore(player, "combat", 380);
	setScore(target, "combat", 280);
}
 ...
 * @returns An array of the slot activation trigger and the slot index number, in that order.
 */
...
export function getBendingStyle(player) {
	const tags = ['air', 'earth', 'water', 'fire', 'avatar'];
	const bendingstyle = tags.find(tag => player.hasTag(tag)) || 'Non-bender';
	return bendingstyle.charAt(0).toUpperCase() + bendingstyle.slice(1);
}

export function canBend(player) {
    const spawn = { x: -1456, y: 66, z: -138 };
    const disableBendingTags = new Set(["antimagic", "bending_off", "server_disabled", "admin_disabled", "in_opp_claim"]);
...
export function levelUp(player) {
    ...
    if (Math.random() > 0.7) player.runCommand(`give @s a:skill_point 1 0 {"minecraft:keep_on_death":{},"minecraft:item_lock":{"mode":"lock_in_inventory"}}`);
...
export function calcBendingResistance(entity) {
    ...
    try {
        ...
        for (const slotName of armorSlots) {
            ...
            if (item) {
                const item_enchants = item.getComponent("enchantable");
...
export function isLineClear(player, startPoint, endPoint) {
    ...
    for (let i = 0; i <= steps; i++) {
        ...
        if (!currentBlock || currentBlock.isSolid) return false;
    }
